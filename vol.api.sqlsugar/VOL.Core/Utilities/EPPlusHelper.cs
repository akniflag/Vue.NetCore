...
namespace VOL.Core.Utilities
{
    public class EPPlusHelper
    {
        ...
        /// <returns></returns>
        public static WebResponseContent ReadToDataTable<T>(
            string path,
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    for (int j = 0; j < propertyInfo.Count; j++)
                    {
                        ...
                        )
                        {
                            ...
                            {
                                cellOptions
                                    .Where(x => x.ColumnName == propertyInfo[j].Name)
                                    .Select(s => s.KeyValues)
                                    .FirstOrDefault()
        ...
        )
        {
            ...
            List<CellOptions> cellOptions = query
                .OrderByDescending(x => x.OrderNo)
                .Select(c => new CellOptions()
                {
            ...
            foreach (string dicNo in dicNos.Distinct())
            {
                ...
                foreach (var item in dictionaryLists)
                {
                    ...
                    if (filterKeyValue && item.DicName == item.DicValue)
                        continue;
