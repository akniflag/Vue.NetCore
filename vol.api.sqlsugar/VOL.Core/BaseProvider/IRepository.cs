...
namespace VOL.Core.BaseProvider
{
        ...
        where TEntity : BaseEntity
    {
        ...
        /// <param name="predicate"></param>
        /// <returns></returns>
        ISugarQueryable<T> WhereIF<T>(bool checkCondition, Expression<Func<T, bool>> predicate)
            where T : class;
        ...
        /// <param name="filterDeleted">是否过滤逻辑删除的数据，默认过</param>
        /// <returns></returns>
        List<TFind> Find<TFind>(Expression<Func<TFind, bool>> predicate, bool filterDeleted = true)
            where TFind : class;
        ...
        /// <param name="filterDeleted">是否过滤逻辑删除的数据，默认过</param>
        /// <returns></returns>
        bool Exists<TExists>(Expression<Func<TExists, bool>> predicate, bool filterDeleted = true)
            where TExists : class;
        ...
        ISugarQueryable<TEntity> IQueryablePage(
            ISugarQueryable<TEntity> queryable,
            int pageIndex,
            int pagesize,
            out int rowcount,
            Dictionary<string, QueryOrderBy> orderBy,
            bool returnRowCount = true
        );
        ...
        int Update<TSource>(TSource entity, string[] properties, bool saveChanges = false)
            where TSource : class, new();

        int UpdateRange<TSource>(IEnumerable<TSource> entities, bool saveChanges = false)
            where TSource : class, new();
        ...
        /// <returns></returns>
        WebResponseContent UpdateRange<Detail>(
            TEntity entity,
        ...
        void Delete<T>(T model, bool saveChanges = false)
            where T : class, new();
        ...
        void AddWithSetIdentity<T>(T entity)
            where T : class, new();

        void Add(TEntity entities, bool SaveChanges = false);

        void Add<T>(T entities, bool saveChanges = false)
            where T : class, new();
